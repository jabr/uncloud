...
func (cli *Client) ListVolumes(ctx context.Context, filter *api.VolumeFilter) ([]api.MachineVolume, error) {
	...
	for _, mv := range machineVolumes {
		...
		if mv.Metadata.Error != "" {
			tui.PrintWarning(fmt.Sprintf("failed to list volumes on machine %s: %s", mv.Metadata.Machine, mv.Metadata.Error))
			continue
		}
		...
		if machineID == "" {
			...
			tui.PrintWarning(fmt.Sprintf("machine ID missing in response from: %s", mv.Metadata.Machine))
			continue
		}
